      ...
      {steps.map((s, i) => (
        <div key={s.n} className="flex items-center">
          <div className="flex items-center gap-2">
            ...
            <span className={cn('text-sm font-medium', current === s.n ? 'text-zinc-900' : 'text-zinc-400')}>
              {s.label}
            </span>
          </div>
          {i < steps.length - 1 && (
            <div className={cn('mx-4 h-px w-12', current > s.n ? 'bg-green-200' : 'bg-zinc-200')} />
          )}
        </div>
      ))}
    </div>
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => router.back()} className="flex items-center gap-1 text-sm text-zinc-400 hover:text-zinc-700">
          <ArrowLeft size={14} />
        </button>
        <div>
          <h2 className="text-xl font-semibold text-zinc-900">New Campaign</h2>
          <p className="text-sm text-zinc-400">Create and launch a personalized campaign</p>
        </div>
      </div>
      ...
      {step === 1 && (
        <div className="space-y-3">
          <p className="text-sm font-semibold text-zinc-700">Select a Segment</p>
          {segmentsLoading ? (
            Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="animate-pulse h-16 rounded-lg border border-zinc-200 bg-zinc-50" />
            ))
          ) : (
              ...
              {segments?.map((seg) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <p className="font-medium text-zinc-900">{seg.name}</p>
                    <div className="flex items-center gap-1 text-sm text-zinc-400">
                      <Users size={13} />
                      {seg.audience_size.toLocaleString()}
                    </div>
                  </div>
                  {seg.description && <p className="mt-0.5 text-xs text-zinc-400 line-clamp-1">{seg.description}</p>}
                </button>
              ))}
            </div>
          )}
      ...
      {step === 2 && (
        <div className="space-y-4">
          <p className="text-sm font-semibold text-zinc-700">Choose Channel</p>
            ...
            {CHANNELS.map((ch) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="mt-1 text-xs text-zinc-400">Expected open rate: <span className="font-medium text-zinc-600">{cfg.openRate}</span></p>
                </button>
              );
            })}
          </div>
      ...
      {step === 3 && (
        <div className="space-y-4">
          <div>
            <label className="mb-1.5 block text-sm font-medium text-zinc-700">Campaign Name</label>
            <input
              value={campaignName}
              onChange={(e) => setCampaignName(e.target.value)}
              placeholder="e.g. Monsoon Re-engagement"
              className="w-full rounded-md border border-zinc-200 px-3 py-2 text-sm outline-none focus:border-blue-400"
            />
          </div>
          ...
          <div>
            <div className="mb-1.5 flex items-center justify-between">
              ...
              <div className="flex gap-1">
                {['{{name}}', '{{city}}', '{{total_spend}}'].map((v) => (
                  <button key={v} onClick={() => insertVariable(v)} className="rounded bg-zinc-100 px-1.5 py-0.5 font-mono text-[11px] text-zinc-600 hover:bg-zinc-200">
                    {v}
                  </button>
                ))}
              </div>
            ...
            <textarea
              ...
              className="w-full resize-none rounded-md border border-zinc-200 px-3 py-2 font-mono text-sm outline-none focus:border-blue-400"
            />
            <p className="mt-1 text-right text-xs text-zinc-400">{message.length} characters</p>
          </div>

          <div className="rounded-lg border border-purple-100 bg-purple-50 p-4 space-y-3">
            <div className="flex items-center gap-2">
              <Sparkles size={13} className="text-purple-600" />
              <p className="text-sm font-medium text-purple-700">AI Message Assistant</p>
            </div>
            <textarea
              value={aiPrompt}
              ...
              className="w-full resize-none rounded-md border border-purple-200 bg-white px-3 py-2 text-sm outline-none focus:border-purple-400"
            />
            <button
          ...
          <div className="flex justify-between">
            <button onClick={() => setStep(2)} className="text-sm text-zinc-500 hover:text-zinc-700">Back</button>
            <button
          ...
          <div className="rounded-lg border border-zinc-200 bg-white p-5 space-y-4">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between py-2 border-b border-zinc-50">
                ...
                <span className="font-medium text-zinc-900">{campaignName}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-zinc-50">
                <span className="text-zinc-400">Segment</span>
                <span className="font-medium text-zinc-900">{selectedSegment?.name}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-zinc-50">
                <span className="text-zinc-400">Audience</span>
                <span className="font-medium text-zinc-900">{selectedSegment?.audience_size.toLocaleString()} customers</span>
              </div>
              <div className="flex justify-between py-2 border-b border-zinc-50">
            ...
            <div>
              <p className="text-xs font-medium text-zinc-400 mb-2 uppercase tracking-wide">Message Preview</p>
              <div className="rounded-md bg-zinc-50 p-3 font-mono text-xs text-zinc-700 whitespace-pre-wrap">{message}</div>
            </div>
          </div>

          <div className="flex items-center justify-between">
            <button onClick={() => setStep(3)} className="text-sm text-zinc-500 hover:text-zinc-700">Back</button>
            <div className="flex gap-2">
              <button onClick={saveDraft} className="rounded-md border border-zinc-200 px-4 py-2 text-sm text-zinc-600 hover:bg-zinc-50">
                Save as Draft
              </button>
              <button
